...
The transform is handled by the other two options, whih can use any valid regex to group paramaters:
...
This is the amtch pattern to use to extract parameters from the URL
...
The service discovery section tells Tyk where to find information about the host to proxy to. In a clustered environment this is useful if servers are coming online and offline dynamically with new IP adresses. The service discovery module can pull out the required host data from any service discovery tool that exposes a RESTful endpoint that outputs a JSON object.
...
Headers taht are allowed within a request
